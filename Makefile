...
up-test: ## Start the docker hub in detached mode (no logs) for testing
	@XDEBUG_MODE=coverage $(DOCKER_COMP) -f compose.yaml -f compose.override.yaml -f compose.test.yaml up --detach --wait
.PHONY: up-test

up-prod: ## Start the docker hub in detached mode (no logs) for production
	@$(DOCKER_COMP) -f compose.yaml -f compose.prod.yaml up --detach --wait
.PHONY: up-prod
...
test-unit: ## Run all unit tests
	@$(DOCKER_COMP) exec -e XDEBUG_MODE=off -e APP_ENV=test php ./vendor/bin/phpunit --no-coverage --testsuite unit --testdox
.PHONY: test-unit

test-functional: ## Run all functional tests
	@$(DOCKER_COMP) exec -e XDEBUG_MODE=off -e APP_ENV=test php ./vendor/bin/phpunit --no-coverage --testsuite functional --testdox
